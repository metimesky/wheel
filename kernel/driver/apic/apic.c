...
void local_apic_init(uint64_t base) {
    ...
    * ((uint64_t *) base + SPURIOUS_INT_VECTOR) = spurious | 0x100;
    ...
    while (U32(base + LVT_TIMER) & (1U << 12)) {}

    // enable timer
    U32(base + LVT_TIMER) = 1UL << 16;

    U32(base + DIVIDE_CONFIGURATION) = 0;   // divide by 2
    U32(base + INITIAL_COUNT) = 0xffffffff; // maximum countdown

    uint64_t tick_a = tick;
    uint32_t count_a = U32(base + CURRENT_COUNT);

    while (tick < tick_a + 50) {}
    uint32_t count_b = U32(base + CURRENT_COUNT);

    // disable timer
    U32(base + LVT_TIMER) = 0UL;

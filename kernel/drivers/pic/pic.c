...
void pic_init() {
    ...
    io_wait();
    out_byte(PIC1_DAT, 32);     // ICW2: map master PIC vector base to 32 (interrupt/interrupt.c)
    io_wait();
    out_byte(PIC2_DAT, 40);     // ICW2: map slave PIC vector base to 40 (interrupt/interrupt.c)
    io_wait();
